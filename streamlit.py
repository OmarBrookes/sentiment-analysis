...
# ----- Setup: Model, Tokenizer, Device, and Configurations -----
...
# ----- Custom Thresholds -----
# Adjusted these values to fine-tune label detection per the model’s performance
custom_thresholds = {
    'neutral': 0.39,
    'positive': 0.509,
    'mixed': 0.22,
...
def analyse_sentiment(text):
    ...
    # Compare each probability against its custom threshold
...
if st.button("Analyse Sentiment"):
    # If user entered text, process that
    if user_input:
        review_count += 1
    ...
    # If a file is uploaded, process each non-empty line as a review
    if uploaded_file:
